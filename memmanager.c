...
{  
   ...
   tlb{current_tlb_entry}{1} = page_talbe[page];
...
if (tlb_index != -1) {
  ...
  // bring data into memory, update tlb and page table
   int available_frame = get_avaliable_fram(page);
    fread(&main_mem_fifo[available_fram*FRAME_SIZE], sizeof(char), 256, fstore);
